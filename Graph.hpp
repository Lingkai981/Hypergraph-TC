...
struct ArrayHash {
    size_t operator()(const std::array<std::vector<unsigned int>, 3>& arr) const {
        size_t hash_value = 0;
        for (const auto& vec : arr) {
            
            size_t vec_hash = 0;
            for (const auto& elem : vec) {
                vec_hash ^= std::hash<unsigned int>()(elem) + 0x9e3779b9 + (vec_hash << 6) + (vec_hash >> 2);
            }
            
            hash_value ^= vec_hash + 0x9e3779b9 + (hash_value << 6) + (hash_value >> 2);
        }
        return hash_value;
    }
};
...
private:
    ...
    std::map<std::array<int, 3>, tri_type_num> tri_type_num_;

public:
    Graph(const char *_dir, uint32_t BUCKET_BITS, double ALPHA);
    ~Graph() ;
    ...
    vector<uint64_t> tc_tri_stream_global_space(uint32_t baseline_rs);
    ...
    vector<uint64_t> HTCount_edge();
    void intersect_two_sets_fixed3(vector<uint32_t> &set1, vector<uint32_t> &set2, int &common_num);

    vector<uint64_t> HTCount();
    vector<uint64_t> HTCount_P();

    vector<uint64_t> HTCount_P_edge();

    void triangle_enum_case_study();
    std::array<int, 3>  normalize_key(int a, int b, int c);
    void judge_tri_type_2_case_study(uint32_t u, uint32_t u_pos, uint32_t v, uint32_t v_pos,uint32_t w, uint32_t w_pos, uint32_t &type1_num, uint32_t &type2_num, uint32_t &common_num);
