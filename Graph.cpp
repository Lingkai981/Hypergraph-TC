...
Graph::Graph(const char *_dir, uint32_t BUCKET_BITS, double ALPHA) {
    dir = string(_dir);
...
void Graph::read_graph() {
    ...
    while (infile >> hyperedge_len[di]){
        ...
        if(min_e > hyperedge_len[di]) min_e = hyperedge_len[di];
        avg_e+=hyperedge_len[di];
        di++;
    }
    infile.close();
    e = di;
...
void Graph::triangle_enum_case_study(){
        ...
        for(uint32_t j = out_edge_start[i]; j < out_edge_start[i+1]; j++){
            ...
            for(uint32_t k = out_edge_start[out_edges[j].vertex]; k < out_edge_start[out_edges[j].vertex+1]; k++){
                ...
                if(visit_out[out_edges[k].vertex] == i+1){
                    ...
                    judge_tri_type_2_case_study(i, out_edges[j].pos, out_edges[j].vertex, out_edges[k].pos, i, visit_out_pos[out_edges[k].vertex], type1_num, type2_num, common_num);
                    // all_tri_num+=tri_num;
                    // type_num[type]+=tri_num;
                    type_num[1]+=type1_num;
                    type_num[2]+=type2_num;

                    tri_type_num_[normalize_key(i, out_edges[j].vertex, out_edges[k].vertex)].hyper = type1_num;
                    tri_type_num_[normalize_key(i, out_edges[j].vertex, out_edges[k].vertex)].outer = type2_num;
                    

                    // cout<<i<<" "<<out_edges[j].vertex<<" "<<out_edges[k].vertex<<" hybrid_triangle:"<<type1_num<<" outer_triangle:"<<type2_num<<endl;
                }
            }
        }
    }
    ...
    cout<<"inner triangles"<<endl;

    for(uint32_t i = 0; i< e;i++) if(hyperedges_start[i+1] - hyperedges_start[i] > 2) type_num[0]+=((hyperedges_start[i+1] - hyperedges_start[i])*(hyperedges_start[i+1] - hyperedges_start[i]-1)*(hyperedges_start[i+1] - hyperedges_start[i]-2))/6;

    for(uint32_t i = 0; i< e;i++){
        if(hyperedges_start[i+1] - hyperedges_start[i] <= 2) continue;
        for(uint32_t j = hyperedges_start[i]; j < hyperedges_start[i+1]-2;j++){
            for(uint32_t j2 = j+1; j2 < hyperedges_start[i+1]-1;j2++){
                for(uint32_t j3 = j2+1; j3 < hyperedges_start[i+1];j3++){
                    
                    tri_type_num_[normalize_key(hyperedges[j], hyperedges[j2], hyperedges[j3])].inner ++;
                }
    ...
    for (const auto& [key, value] : tri_type_num_) { // C++17 结构化绑定
        cout << "{";
        for (int i = 0; i < 3; i++) {
            cout << key[i] << (i < 2 ? ", " : "");
        }
        cout << "} -> inner_triangle:" << value.inner << " hybrid_triangle:"<<value.hyper<<" outer_triangle:"<<value.outer << endl;
    }
...
void Graph::judge_tri_type_2_case_study(uint32_t u, uint32_t u_pos, uint32_t v, uint32_t v_pos,uint32_t w, uint32_t w_pos, uint32_t &type1_num, uint32_t &type2_num, uint32_t &common_num){
...
std::array<int, 3> Graph::normalize_key(int a, int b, int c) {
...
void Graph::intersect_three_sets3(
    ...
    if (update_factor != 0) {
        tri_num_type3_ = ((common_num_1 - common_num_) * (common_num_2 - common_num_) * (common_num_3 - common_num_)) / (update_factor);
    } else {
        tri_num_type3_ = 0;
    }
}
...
void Graph::intersect_two_sets_fixed3(vector<uint32_t> &set1, vector<uint32_t> &set2, int &common_num){
    common_num = 0;
...
vector<uint64_t> Graph::tc_tri_stream_local_space_fixed_muti_Reservoir(){
    ...
    for(int i = 0;i<e;i++){
        // if(all_hyperedges[i].size() < 2) continue;
        // cout<< i<<endl;
        if((double)sampleVertices_all/(double)Reservoir_size_2 < alpha && r_full_muti[Reservoir_num-1] == 1 && Reservoir_num < max_Reservoir_size){
...
vector<uint64_t> Graph::tc_tri_stream_global_space(uint32_t baseline_rs){
    double tri_num_type1 = 0;
    double tri_num_type2 = 0;
    double tri_num_type3 = 0;
    uint32_t currentEdges = 0;
    vector<uint64_t> type_num(4,0);
    ...
    int judge_type1 = 0;
    double update_factor = 1, update_factor2 = 1,update_factor3 = 1;

    baseline_rs = baseline_rs/2;

    std::cout<<"Reservoir_size:"<<baseline_rs<<std::endl;
    vector<uint32_t> edge_nei(e);
    uint32_t edge_nei_i = 0;
    vector<int> edge_nei_exist(e,-1);
    ...
    for(int i = 0;i<e;i++){
        ...
        if (currentEdges <= baseline_rs) {
            size_now += all_hyperedges[i].size();
            if(size_now > Reservoir_size_){
                cout<<"size_now is over "<< size_now<<" "<<i<<endl;
                return type_num;
        ...
        else if (dis(gen) < (double)baseline_rs / (double)currentEdges) {
            uint32_t del_edge = rand() % baseline_rs;
            size_now -= sample_hyperedges[del_edge].size();
    ...
    update_factor2 = ((double)baseline_rs/(double)(currentEdges))*((double)(baseline_rs-1)/(double)(currentEdges-1));
    update_factor3 = ((double)baseline_rs/(double)(currentEdges))*((double)(baseline_rs-1)/(double)(currentEdges-1))*((double)(baseline_rs-2)/(double)(currentEdges-2));
    update_factor = ((double)baseline_rs/(double)(currentEdges));

    for(int i = 0; i < baseline_rs; i++){
        ...
        for(uint32_t j = 0;j<edge_nei_i;j++){
            ...
            if(edge_nei[j] > i){
                intersect_two_sets3(sample_hyperedges[i], sample_hyperedges[edge_nei[j]], tri_num_type2_, update_factor2);
                tri_num_type2+=tri_num_type2_;

                for(uint32_t k = j+1;k<edge_nei_i;k++){
                    if(edge_nei[k] > i){
                        intersect_three_sets3(sample_hyperedges[i], sample_hyperedges[edge_nei[j]], sample_hyperedges[edge_nei[k]], tri_num_type3_, update_factor3);
                        tri_num_type3+=tri_num_type3_;
                    }
                }
    ...
    type_num[1] = (int64_t)tri_num_type2;
    type_num[2] = (int64_t)tri_num_type3;
    type_num[3] = (int64_t)size_now;

    return type_num;
}


vector<uint64_t> Graph::HTCount_edge(){
    double tri_num_CCC = 0;
    double tri_num_TCC = 0;
    double tri_num_TTC = 0;
    double tri_num_TTT = 0;
    uint32_t currentEdges = 0;
    uint32_t currentVertices = 0;
    uint32_t sampleVertices = 0;
    uint32_t Reservoir_size_ = Reservoir_size;
    int wedge1, wedge2, wedge3;
    double temp;
    ...
    for(int i = 0;i<e;i++){
        ...
        update_factor = 1.0;
        if(r_full) update_factor = ((double)(Reservoir_size_now)/(double)(currentEdges))*((double)(Reservoir_size_now-1)/(double)(currentEdges-1))*((double)(Reservoir_size_now-2)/(double)(currentEdges-2));
        ...
        if(all_hyperedges[i].size() >= 1){
            ...
            sample_common_vertices.resize(edge_nei_i);
            for(uint32_t j = 0;j<edge_nei_i;j++){
                intersect_two_sets_fixed2(all_hyperedges[i], sample_hyperedges_fixed[edge_nei[j]], temp, update_factor, common_num, sample_common_vertices[j]);
            }
            for(uint32_t j = 0;j<edge_nei_i;j++){
                if(sample_common_vertices[j].size() == 0) continue;
                if(sample_common_vertices[j].size() == min(all_hyperedges[i].size(), sample_hyperedges_fixed[edge_nei[j]].size())) wedge1 = 0;
                else  wedge1 = 1;
                for(uint32_t k = j+1;k<edge_nei_i;k++){
                    if(sample_common_vertices[k].size() == 0) continue;
                    if(sample_common_vertices[k].size() == min(all_hyperedges[i].size(), sample_hyperedges_fixed[edge_nei[k]].size())) wedge2 = 0;
                    else  wedge2 = 1;

                    intersect_two_sets_fixed3(sample_hyperedges_fixed[edge_nei[j]], sample_hyperedges_fixed[edge_nei[k]], common_num);
                    if(common_num == 0) continue;
                    if(common_num == min(sample_hyperedges_fixed[edge_nei[j]].size(), sample_hyperedges_fixed[edge_nei[k]].size())) wedge3 = 0;
                    else  wedge3 = 1;

                    if(wedge1+wedge2+wedge3 == 0) tri_num_CCC+=1.0/update_factor;
                    else if(wedge1+wedge2+wedge3 == 1) tri_num_TCC+=1.0/update_factor;
                    else if(wedge1+wedge2+wedge3 == 2) tri_num_TTC+=1.0/update_factor;
                    else tri_num_TTT+=1.0/update_factor;
                }
            }
        }
    ...
    vector<uint64_t> type_num(4);
    type_num[0] = tri_num_CCC;
    type_num[1] = tri_num_TCC;
    type_num[2] = tri_num_TTC;
    type_num[3] = tri_num_TTT;
    

    return type_num;
}

vector<uint64_t> Graph::HTCount(){
    double tri_num_type1 = 0;
    double tri_num_type2 = 0;
    double tri_num_type3 = 0;
    uint32_t currentEdges = 0;
    ...
    int judge_type1 = 0;
    double update_factor2 = 1.0, update_factor3 = 1.0;
    vector<uint32_t> edge_nei(min(Reservoir_size_/2, e));
    uint32_t edge_nei_i = 0;
    vector<int> edge_nei_dup(e,-1);
    vector<pairwise_edge> sample_all_vertices;
    ...
    for(int i = 0;i<e;i++){
        ...
        if (currentVertices <= Reservoir_size_) {
            ...
            for(uint32_t j = 0;j<edge_nei_i;j++){
                intersect_two_sets_fixed2(all_hyperedges[i], sample_hyperedges_fixed[edge_nei[j]], tri_num_type2_, 1.0, common_num, sample_common_vertices[j]);
                tri_num_type2+=tri_num_type2_;
            ...
            for(uint32_t j = 0;j<edge_nei_i;j++){
                ...
                for(uint32_t k = j+1;k<edge_nei_i;k++){
                    if(all_hyperedges[i].size() - sample_common_vertices[k].size() == 0 || sample_hyperedges_fixed[edge_nei[k]].size() - sample_common_vertices[k].size() == 0) continue;
                    intersect_three_sets_fixed2(sample_common_vertices[j], sample_common_vertices[k], sample_hyperedges_fixed[edge_nei[j]], sample_hyperedges_fixed[edge_nei[k]], tri_num_type3_, 1.0);
                    tri_num_type3+=tri_num_type3_;
                }
            }
            }
            ...
            update_factor2 = ((double)Reservoir_size_now*(double)(Reservoir_size_now-1)/(double)(currentEdges)/(double)(currentEdges-1));
            update_factor3 = ((double)Reservoir_size_now*(double)(Reservoir_size_now-1)*(double)(Reservoir_size_now-2)/(double)(currentEdges)/(double)(currentEdges-1)/(double)(currentEdges-2));
            ...
            for(uint32_t j = 0;j<edge_nei_i;j++){
                intersect_two_sets_fixed2(all_hyperedges[i], sample_hyperedges_fixed[edge_nei[j]], tri_num_type2_, update_factor2, common_num, sample_common_vertices[j]);
                tri_num_type2+=tri_num_type2_;
            ...
            for(uint32_t j = 0;j<edge_nei_i;j++){
                ...
                for(uint32_t k = j+1;k<edge_nei_i;k++){
                    if(all_hyperedges[i].size() - sample_common_vertices[k].size() == 0 || sample_hyperedges_fixed[edge_nei[k]].size() - sample_common_vertices[k].size() == 0) continue;
                    intersect_three_sets_fixed2(sample_common_vertices[j], sample_common_vertices[k], sample_hyperedges_fixed[edge_nei[j]], sample_hyperedges_fixed[edge_nei[k]], tri_num_type3_, update_factor3);
                    tri_num_type3+=tri_num_type3_;
                }
            }
            }
...
vector<uint64_t> Graph::HTCount_P(){
    double tri_num_type1 = 0;
    double tri_num_type2 = 0;
    double tri_num_type3 = 0;
    // uint32_t currentEdges = 0;
    ...
    double update_factor2 = 1,update_factor3 = 1;
    ...
    for(int i = 0;i<e;i++){
        ...
        // cout<< i<<endl;
        if(((double)sampleVertices_all/(double)Reservoir_size_2 < alpha) && (r_full_muti[Reservoir_num-1] == 1) && (Reservoir_num < max_Reservoir_size)){
            cout<<(double)sampleVertices_all/(double)Reservoir_size_2 << " "<< r_full_muti[Reservoir_num-1]<<" "<<Reservoir_num<<" "<< max_Reservoir_size<<endl;
            // if(Reservoir_size_muti[Reservoir_num-1] < )
            // cout<<sampleVertices_all<<" "<<Reservoir_size_2<<endl;
            Reservoir_num++;
            r_full_muti[Reservoir_num-1] = 0;
            sampleVertices_muti[Reservoir_num-1] = 0;
            currentEdges_muti[Reservoir_num - 1] = 0;
            currentVertices_muti[Reservoir_num - 1] = 0;
            ...
            for(int Reservoir_i = 0; Reservoir_i<Reservoir_num-1; Reservoir_i++){
                Reservoir_size_muti[Reservoir_i] = sampleVertices_muti[Reservoir_i]+3;
                Reservoir_size_muti_old+=Reservoir_size_muti[Reservoir_i];
            } 
            Reservoir_size_muti[Reservoir_num-1] = Reservoir_size_2 - Reservoir_size_muti_old;
            Reservoir_size_now_muti[Reservoir_num - 1] = 0;
            sample_hyperedges_fixed_muti_start[Reservoir_num - 1] = sample_hyperedges_fixed_muti_start[Reservoir_num - 2] + Reservoir_size_now_muti[Reservoir_num - 2];
            cout<<Reservoir_num<<" "<<Reservoir_size_muti[Reservoir_num-1]<<" "<<Reservoir_size_muti_old<<" "<<Reservoir_size_2<<endl;
            if(Reservoir_size_muti[Reservoir_num-1] < 1){ 
                max_Reservoir_size = Reservoir_num - 1;
                Reservoir_num--;
            }
        }
        ...
        if (currentVertices <= Reservoir_size_) {
            ...
            for(uint32_t j = 0;j<edge_nei_i;j++){
                if(reservoir_ownership[edge_nei[j]] == current_Reservoir_id) update_factor2 = 1.0;
                else update_factor2 = ((double)Reservoir_size_now_muti[reservoir_ownership[edge_nei[j]]]/(double)(currentEdges_muti[reservoir_ownership[edge_nei[j]]]));
                intersect_two_sets_fixed2(all_hyperedges[i], sample_hyperedges_fixed[edge_nei[j]], tri_num_type2_, update_factor2, common_num, sample_common_vertices[j]);
                tri_num_type2+=tri_num_type2_;
            ...
            for(uint32_t j = 0;j<edge_nei_i;j++){
                ...
                for(uint32_t k = j+1;k<edge_nei_i;k++){
                    ...
                    if(current_Reservoir_id == reservoir_ownership[edge_nei[j]] && reservoir_ownership[edge_nei[j]] == reservoir_ownership[edge_nei[k]]) update_factor3 = 1.0;

                    else if(current_Reservoir_id == reservoir_ownership[edge_nei[j]] && current_Reservoir_id != reservoir_ownership[edge_nei[k]]) update_factor3 = ((double)Reservoir_size_now_muti[reservoir_ownership[edge_nei[k]]]/(double)(currentEdges_muti[reservoir_ownership[edge_nei[k]]]));
                    else if(current_Reservoir_id == reservoir_ownership[edge_nei[k]] && current_Reservoir_id != reservoir_ownership[edge_nei[j]]) update_factor3 = ((double)Reservoir_size_now_muti[reservoir_ownership[edge_nei[j]]]/(double)(currentEdges_muti[reservoir_ownership[edge_nei[j]]]));
                    else if(current_Reservoir_id != reservoir_ownership[edge_nei[k]] && current_Reservoir_id != reservoir_ownership[edge_nei[j]]){
                        if(reservoir_ownership[edge_nei[j]] == reservoir_ownership[edge_nei[k]]){
                            update_factor3 = ((double)Reservoir_size_now_muti[reservoir_ownership[edge_nei[k]]]*((double)Reservoir_size_now_muti[reservoir_ownership[edge_nei[k]]]-1)/(double)(currentEdges_muti[reservoir_ownership[edge_nei[k]]])/((double)(currentEdges_muti[reservoir_ownership[edge_nei[k]]])-1));
                        }else {
                            update_factor3 = ((double)Reservoir_size_now_muti[reservoir_ownership[edge_nei[k]]]/(double)(currentEdges_muti[reservoir_ownership[edge_nei[k]]]))*((double)Reservoir_size_now_muti[reservoir_ownership[edge_nei[j]]]/(double)(currentEdges_muti[reservoir_ownership[edge_nei[j]]]));
                        }
                    }
                    intersect_three_sets_fixed2(sample_common_vertices[j], sample_common_vertices[k], sample_hyperedges_fixed[edge_nei[j]], sample_hyperedges_fixed[edge_nei[k]], tri_num_type3_, update_factor3);
                    tri_num_type3+=tri_num_type3_;
            ...
            for(uint32_t j = 0;j<edge_nei_i;j++){
                if(reservoir_ownership[edge_nei[j]] == current_Reservoir_id) update_factor2 = ((double)Reservoir_size_now_muti[reservoir_ownership[edge_nei[j]]]*(double)(Reservoir_size_now_muti[reservoir_ownership[edge_nei[j]]]-1)/(double)(currentEdges_muti[reservoir_ownership[edge_nei[j]]])/(double)(currentEdges_muti[reservoir_ownership[edge_nei[j]]]-1));
                else update_factor2 = ((double)Reservoir_size_now_muti[reservoir_ownership[edge_nei[j]]]*(double)Reservoir_size_now_muti[current_Reservoir_id]/(double)(currentEdges_muti[reservoir_ownership[edge_nei[j]]])/(double)(currentEdges_muti[current_Reservoir_id]));
                intersect_two_sets_fixed2(all_hyperedges[i], sample_hyperedges_fixed[edge_nei[j]], tri_num_type2_, update_factor2, common_num, sample_common_vertices[j]);
                tri_num_type2+=tri_num_type2_;
            ...
            for(uint32_t j = 0;j<edge_nei_i;j++){
                ...
                for(uint32_t k = j+1;k<edge_nei_i;k++){
                    ...
                    if(current_Reservoir_id == reservoir_ownership[edge_nei[j]] && reservoir_ownership[edge_nei[j]] == reservoir_ownership[edge_nei[k]]) update_factor3 = ((double)(Reservoir_size_now_muti[current_Reservoir_id])/(double)(currentEdges_muti[current_Reservoir_id]))*((double)(Reservoir_size_now_muti[current_Reservoir_id]-1)/(double)(currentEdges_muti[current_Reservoir_id]-1))*((double)(Reservoir_size_now_muti[current_Reservoir_id]-2)/(double)(currentEdges_muti[current_Reservoir_id]-2));

                    else if(current_Reservoir_id == reservoir_ownership[edge_nei[j]] && current_Reservoir_id != reservoir_ownership[edge_nei[k]]) update_factor3 = ((double)(Reservoir_size_now_muti[current_Reservoir_id])/(double)(currentEdges_muti[current_Reservoir_id]))*((double)(Reservoir_size_now_muti[current_Reservoir_id]-1)/(double)(currentEdges_muti[current_Reservoir_id]-1))*((double)Reservoir_size_now_muti[reservoir_ownership[edge_nei[k]]]/(double)(currentEdges_muti[reservoir_ownership[edge_nei[k]]]));
                    else if(current_Reservoir_id == reservoir_ownership[edge_nei[k]] && current_Reservoir_id != reservoir_ownership[edge_nei[j]]) update_factor3 = ((double)(Reservoir_size_now_muti[current_Reservoir_id])/(double)(currentEdges_muti[current_Reservoir_id]))*((double)(Reservoir_size_now_muti[current_Reservoir_id]-1)/(double)(currentEdges_muti[current_Reservoir_id]-1))*((double)Reservoir_size_now_muti[reservoir_ownership[edge_nei[j]]]/(double)(currentEdges_muti[reservoir_ownership[edge_nei[j]]]));
                    else if(current_Reservoir_id != reservoir_ownership[edge_nei[k]] && current_Reservoir_id != reservoir_ownership[edge_nei[j]]){
                        if(reservoir_ownership[edge_nei[j]] == reservoir_ownership[edge_nei[k]]){
                            update_factor3 = ((double)(Reservoir_size_now_muti[current_Reservoir_id])/(double)(currentEdges_muti[current_Reservoir_id]))*((double)Reservoir_size_now_muti[reservoir_ownership[edge_nei[k]]]*((double)Reservoir_size_now_muti[reservoir_ownership[edge_nei[k]]]-1)/(double)(currentEdges_muti[reservoir_ownership[edge_nei[k]]])/((double)(currentEdges_muti[reservoir_ownership[edge_nei[k]]])-1));
                        }else {
                            update_factor3 = ((double)(Reservoir_size_now_muti[current_Reservoir_id])/(double)(currentEdges_muti[current_Reservoir_id]))*((double)Reservoir_size_now_muti[reservoir_ownership[edge_nei[k]]]/(double)(currentEdges_muti[reservoir_ownership[edge_nei[k]]]))*((double)Reservoir_size_now_muti[reservoir_ownership[edge_nei[j]]]/(double)(currentEdges_muti[reservoir_ownership[edge_nei[j]]]));
                        }
                    }
                    intersect_three_sets_fixed2(sample_common_vertices[j], sample_common_vertices[k], sample_hyperedges_fixed[edge_nei[j]], sample_hyperedges_fixed[edge_nei[k]], tri_num_type3_, update_factor3);
                    tri_num_type3+=tri_num_type3_;
...
vector<uint64_t> Graph::HTCount_P_edge(){
    double tri_num_CCC = 0;
    double tri_num_TCC = 0;
    double tri_num_TTC = 0;
    double tri_num_TTT = 0;
        ...
        if(all_hyperedges[i].size() >= 1){
            ...
            for(uint32_t j = 0;j<edge_nei_i;j++){
                ...
                for(uint32_t k = j+1;k<edge_nei_i;k++){
                    ...
                    if(current_Reservoir_id == reservoir_ownership[edge_nei[j]] && reservoir_ownership[edge_nei[j]] == reservoir_ownership[edge_nei[k]]) update_factor = 1.0;

                    else if(current_Reservoir_id == reservoir_ownership[edge_nei[j]] && current_Reservoir_id != reservoir_ownership[edge_nei[k]]) update_factor = ((double)Reservoir_size_now_muti[reservoir_ownership[edge_nei[k]]]/(double)(currentEdges_muti[reservoir_ownership[edge_nei[k]]]));
                    else if(current_Reservoir_id == reservoir_ownership[edge_nei[k]] && current_Reservoir_id != reservoir_ownership[edge_nei[j]]) update_factor = ((double)Reservoir_size_now_muti[reservoir_ownership[edge_nei[j]]]/(double)(currentEdges_muti[reservoir_ownership[edge_nei[j]]]));
                    else if(current_Reservoir_id != reservoir_ownership[edge_nei[k]] && current_Reservoir_id != reservoir_ownership[edge_nei[j]]){
                        if(reservoir_ownership[edge_nei[j]] == reservoir_ownership[edge_nei[k]]){
                            update_factor = ((double)Reservoir_size_now_muti[reservoir_ownership[edge_nei[k]]]*((double)Reservoir_size_now_muti[reservoir_ownership[edge_nei[k]]]-1)/(double)(currentEdges_muti[reservoir_ownership[edge_nei[k]]])/((double)(currentEdges_muti[reservoir_ownership[edge_nei[k]]])-1));
                        }else {
                            update_factor = ((double)Reservoir_size_now_muti[reservoir_ownership[edge_nei[k]]]/(double)(currentEdges_muti[reservoir_ownership[edge_nei[k]]]))*((double)Reservoir_size_now_muti[reservoir_ownership[edge_nei[j]]]/(double)(currentEdges_muti[reservoir_ownership[edge_nei[j]]]));
                        }
                    }
        ...
        if(all_hyperedges[i].size() >= 1){
            ...
            for(uint32_t j = 0;j<edge_nei_i;j++){
                ...
                for(uint32_t k = j+1;k<edge_nei_i;k++){
                    ...
                    if(current_Reservoir_id == reservoir_ownership[edge_nei[j]] && reservoir_ownership[edge_nei[j]] == reservoir_ownership[edge_nei[k]]) update_factor = ((double)(Reservoir_size_now_muti[current_Reservoir_id])/(double)(currentEdges_muti[current_Reservoir_id]))*((double)(Reservoir_size_now_muti[current_Reservoir_id]-1)/(double)(currentEdges_muti[current_Reservoir_id]-1))*((double)(Reservoir_size_now_muti[current_Reservoir_id]-2)/(double)(currentEdges_muti[current_Reservoir_id]-2));

                    else if(current_Reservoir_id == reservoir_ownership[edge_nei[j]] && current_Reservoir_id != reservoir_ownership[edge_nei[k]]) update_factor = ((double)(Reservoir_size_now_muti[current_Reservoir_id])/(double)(currentEdges_muti[current_Reservoir_id]))*((double)(Reservoir_size_now_muti[current_Reservoir_id]-1)/(double)(currentEdges_muti[current_Reservoir_id]-1))*((double)Reservoir_size_now_muti[reservoir_ownership[edge_nei[k]]]/(double)(currentEdges_muti[reservoir_ownership[edge_nei[k]]]));
                    else if(current_Reservoir_id == reservoir_ownership[edge_nei[k]] && current_Reservoir_id != reservoir_ownership[edge_nei[j]]) update_factor = ((double)(Reservoir_size_now_muti[current_Reservoir_id])/(double)(currentEdges_muti[current_Reservoir_id]))*((double)(Reservoir_size_now_muti[current_Reservoir_id]-1)/(double)(currentEdges_muti[current_Reservoir_id]-1))*((double)Reservoir_size_now_muti[reservoir_ownership[edge_nei[j]]]/(double)(currentEdges_muti[reservoir_ownership[edge_nei[j]]]));
                    else if(current_Reservoir_id != reservoir_ownership[edge_nei[k]] && current_Reservoir_id != reservoir_ownership[edge_nei[j]]){
                        if(reservoir_ownership[edge_nei[j]] == reservoir_ownership[edge_nei[k]]){
                            update_factor = ((double)(Reservoir_size_now_muti[current_Reservoir_id])/(double)(currentEdges_muti[current_Reservoir_id]))*((double)Reservoir_size_now_muti[reservoir_ownership[edge_nei[k]]]*((double)Reservoir_size_now_muti[reservoir_ownership[edge_nei[k]]]-1)/(double)(currentEdges_muti[reservoir_ownership[edge_nei[k]]])/((double)(currentEdges_muti[reservoir_ownership[edge_nei[k]]])-1));
                        }else {
                            update_factor = ((double)(Reservoir_size_now_muti[current_Reservoir_id])/(double)(currentEdges_muti[current_Reservoir_id]))*((double)Reservoir_size_now_muti[reservoir_ownership[edge_nei[k]]]/(double)(currentEdges_muti[reservoir_ownership[edge_nei[k]]]))*((double)Reservoir_size_now_muti[reservoir_ownership[edge_nei[j]]]/(double)(currentEdges_muti[reservoir_ownership[edge_nei[j]]]));
                        }
                    }
